...
class pre_picking(models.Model):
    ...
    usuario_asignado =  fields.Many2one('zones','Surtidor')
    ...
    def _recuperar_datos(self):
        ...
        if self.so_asignado:
            ...
            if existen_pre_picking : 
                ultimo_id = self.env['pre_picking'].search_read([])[-1]['usuario_asignado'][0]
                self.usuario_asignado = ultimo_id
            ...
            out_done_date = self.env['stock.picking'].search([('name', '=', self.out_name)]).date_done
    ...
    @api.one
    @api.depends('usuario_asignado')
    def _usuario_asignado(self):
        _logger = logging.getLogger(__name__) 
        usuario = self.usuario_asignado
        _logger.info('Usuario: %s ', usuario )
        zona = self.env['zones'].search([('zone', '=', usuario.zone)]).zone
        marcas = self.env['zones'].search([('zone', '=', usuario.zone)]).marcas_zone
        self.zona_asignada = zona
        self.marcas_asignadas = marcas
